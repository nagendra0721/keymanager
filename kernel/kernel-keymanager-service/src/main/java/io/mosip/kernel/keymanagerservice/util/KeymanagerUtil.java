...
public class KeymanagerUtil {
	...
	public byte[] encryptKey(PrivateKey privateKey, PublicKey masterKey) {
		...
		if (masterKey.getAlgorithm().equalsIgnoreCase(KeymanagerConstant.RSA)) {
			...
			encryptedKey = ecCryptomanagerService.asymmetricEcEncrypt(masterKey, privateKey.getEncoded(),
					getEcCurveName(masterKey));
		}
		return encryptedKey;
	}
		...
		try {
			...
			if (Objects.isNull(pemObject)) {
				...
				throw new KeymanagerServiceException(
						io.mosip.kernel.keymanagerservice.constant.KeymanagerErrorConstant.CERTIFICATE_PARSING_ERROR
								.getErrorCode(),
	...
	public CertificateParameters getCertificateParameters(X500Principal latestCertPrincipal, LocalDateTime notBefore,
	...
	public CertificateParameters getCertificateParametersIncludeSAN(X500Principal latestCertPrincipal,
			LocalDateTime notBefore, LocalDateTime notAfter, Map<String, String> altValuesMap) {
	...
	public CertificateParameters getCertificateParameters(KeyPairGenerateRequestDto request, LocalDateTime notBefore,
			LocalDateTime notAfter,
			String appId) {
	...
	public ExtendedCertificateParameters getCertificateParametersIncludeSAN(KeyPairGenerateRequestDto request,
	...
	public CertificateParameters getCertificateParameters(CSRGenerateRequestDto request, LocalDateTime notBefore,
			LocalDateTime notAfter) {
	...
	public CertificateParameters getCertificateParameters(String cName, LocalDateTime notBefore,
			LocalDateTime notAfter) {
	...
	public String getCSR(PrivateKey privateKey, PublicKey publicKey, CertificateParameters certParams,
		...
		try {
			X500Principal csrSubject = new X500Principal(
					"CN=" + certParams.getCommonName() + ", OU=" + certParams.getOrganizationUnit() +
							", O=" + certParams.getOrganization() + ", L=" + certParams.getLocation() +
							", S=" + certParams.getState() + ", C=" + certParams.getCountry());
			ContentSigner contentSigner;
			String privateKeyAlgo = privateKey.getAlgorithm();
			if (privateKeyAlgo.equals(KeymanagerConstant.ED25519_KEY_TYPE) ||
				...
				contentSigner = new JcaContentSignerBuilder(getSignatureAlgorithm(keyAlgorithm))
						.setProvider(keyStore.getKeystoreProviderName()).build(privateKey);
			}
			PKCS10CertificationRequestBuilder pcks10Builder = new JcaPKCS10CertificationRequestBuilder(csrSubject,
					publicKey);
	...
	@SuppressWarnings("java:S4790") // added suppress for sonarcloud, sha1 hash is used for value identification
									// only not for any sensitive data.
	...
	private Map<String, Set<?>> getTrustAnchors() {
		...
		keyAliasRepository.findByReferenceId(KeymanagerConstant.EMPTY).forEach(trustCert -> {
			try {
				...
				LOGGER.warn(KeymanagerConstant.SESSIONID, KeymanagerConstant.APPLICATIONID,
						trustCert.getApplicationId(),
						KeymanagerConstant.REFERENCEID, trustCert.getReferenceId(), "Skip trustCert due to error: {}",
						e.getMessage());
			}

			keyAliasRepository.findByApplicationIdAndReferenceId(signApplicationid, certificateSignRefID)
					.forEach(signCert -> {
						Optional<String> optionalRefId = Optional.ofNullable(signCert.getReferenceId());
						AllCertificatesDataResponseDto certData = keymanagerService
								.getAllCertificates(signCert.getApplicationId(), optionalRefId);

						for (CertificateDataResponseDto cert : certData.getAllCertificates()) {
							X509Certificate x509Certificate = (X509Certificate) convertToCertificate(
	...
	public List<? extends Certificate> getCertificateTrustPath(X509Certificate reqX509Cert) {
		...
		try {
			Map<String, Set<?>> trustStoreMap = (Map<String, Set<?>>) keyAliasTrustAnchorsCache
					.get(KeymanagerConstant.DEFAULT_VALUE);
